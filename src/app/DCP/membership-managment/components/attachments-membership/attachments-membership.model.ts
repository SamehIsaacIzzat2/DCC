...
export class AttachmentsModel {
    ...
    saveData(){
        this.attachments.forEach(attachment=>{
            if((attachment.isMandatory&&attachment.files.length==0)||(attachment.filesCount&&attachment.files.length!=attachment.filesCount)){
    ...
    getRequiredDocuments(){
        const servicesConfigs$ = this.lookupSer.getServicesConfigs("New Membership");
        servicesConfigs$.pipe(takeUntil(this.endSub$)).subscribe((configs) => {
            this.apiSer.get(APIs.membership.requiredDocuments + "/"+configs[0].id).subscribe(data => {
                if (!data.isError) {
    ...
    public upload(files:FileData[]){
        for (let index = 0; index < this.attachments.length; index++) {
            for (let index2 = 0; index2 < files.length; index2++) {
                if(this.attachments[index].documentName == files[index2].documentName){
